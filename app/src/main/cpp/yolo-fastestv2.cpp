...
yoloFastestv2::yoloFastestv2() {
    ...
    //anchor box w h
    std::vector<float> bias{86.55, 82.01, 94.60, 118.83, 114.22, 140.79,
                            139.28, 164.70, 140.64, 102.96, 294.30, 190.42};
...
//ncnn 模型加载
int yoloFastestv2::loadModel(const char *paramPath, const char *binPath, AAssetManager *mmgr) {
    printf("Ncnn mode init:\n%s\n%s\n", paramPath, binPath);

    int ret = net.load_param(mmgr, paramPath);
    ...
    ret = net.load_model(mmgr, binPath);
    if(ret != 0){
...
int
yoloFastestv2::detection(ncnn::Mat inputImg, std::vector<TargetBox> &dstBoxes, const float thresh) {
    dstBoxes.clear();
    ...
    //特征图后处理
    predHandle(out, tmpBoxes, thresh);
...
void yoloFastestv2::calScale(const int width, const int height) {
    scaleW = (float) width / (float) inputWidth;
    scaleH = (float) height / (float) inputHeight;
}
